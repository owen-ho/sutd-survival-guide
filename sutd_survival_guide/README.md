...
| 📅 Deadline Notifier | `dylan_deadline_notifier/` | List / Upcoming / Modules / Stats wired (reads same `bot_data.json`). Add flows routed to hints. |
...
features/
  ...
  deadlines.py    # reads Dylan's bot_data.json
...
- Deadline **add module/exam/homework** as guided conversations.
- Last Train **trip planner** flow (campus location → leave time → station).
- Porting Dylan's 12-hour reminder job onto the unified bot's `JobQueue`.
